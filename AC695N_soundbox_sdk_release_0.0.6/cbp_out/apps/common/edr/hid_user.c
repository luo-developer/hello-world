...
static u32 user_data_write_sub(u8 *data, u16 len)
{
    ...
    if (len + 2 > cbuf_get_space(&user_send_cbuf)) {
        return 0;
    }
...
static u8 need_clear_key = 0;

static void hid_key_send(u16 key)
{
    ...
    putchar('$');
    key_info.report_type = HID_DATA | DATA_INPUT;
    key_info.report_id = 1;
    key_info.key_value = key;
    user_hid_send_data((u8 *)&key_info, 4);
}
...
//5、把const u8 hid_conn_depend_on_dev_company的值置1;
#endif

...
#include "typedef.h"
#include "btstack/dueros_dma.h"
#include "btstack/dueros_dma_pb.h"
#include "btstack/dueros_platform.h"
...
#define DMA_SYS_MEM_LEN    512//1024
...
extern void set_dma_need_speech(u8 need);
#endif
        ...
        if (set_dueros_pair_state(PAIR_STEPS_SUCESS, 0)) {
            ...
            if (get_tws_sibling_connect_state()) {
                if (get_curr_channel_state()&A2DP_CH) {
                    tws_api_sync_call_by_uuid('T', SYNC_CMD_DMA_CONNECTED_ALL_FINISH_TONE, TWS_SYNC_TIME_DO);
                } else {
                    tws_api_sync_call_by_uuid('T', SYNC_CMD_NEED_BT_TONE, TWS_SYNC_TIME_DO);
                }
            } else
#endif
            {
                if (get_curr_channel_state()&A2DP_CH) {
                    dueros_event_post(SYS_BT_AI_EVENT_TYPE_STATUS, KEY_DMA_CONNECTED_ALL_FINISH);
                } else {
                    dueros_event_post(SYS_BT_AI_EVENT_TYPE_STATUS, KEY_DMA_NEED_CON_BT);
                }
            }
#endif
        } else {
...
static volatile u8 spp_connect_flg = 0;
...
bool dma_msg_deal(u32 param)
{
    ...
    switch (param) {
    case APP_DUEROS_VER:
        printf("msg_dueros_ver\n");
        dueros_dma_send_ver();
        set_dueros_pair_state(PAIR_STEPS_INIT, 1);
        break;

    case KEY_SEND_SPEECH_START:
        printf("--MSG_SEND_SPEECH_START\n");

        printf("bt connect staturs:%d \n", get_bt_connect_status());

        if ((BT_STATUS_CONNECTING != get_bt_connect_status()) && (BT_STATUS_PLAYING_MUSIC != get_bt_connect_status())) {
            printf("bt not connect \n");
            break;
        }

        ret = dueros_dma_start_speech();
        if (ret == 0) {
#if 0
            if (!tone_get_status()) {
                tone_play_index(IDEX_TONE_SPEECH_START);
            }
#endif
        } else {
            printf("dueros_dma_start_speech error !!! \n");
        }
        break;

    case APP_TWS_BLE_SLAVE_SPEECH_START:
        ...
        printf("--MSG_TWS_BLE_SPEECH_START\n");
        dueros_start_speech();

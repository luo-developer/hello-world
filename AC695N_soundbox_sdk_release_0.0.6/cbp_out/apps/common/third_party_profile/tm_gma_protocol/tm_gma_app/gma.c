...
typedef enum {
    ...
    COMMAND_HEARTBEAT			= 0X50,  //app->device
} GMA_CMD_E;
...
typedef struct __gma_ota_packet_hdl
{
	struct _data_buf
	{
		u8 *buffer;
	...
	u8 ota_buf_init;
	u32 last_packet_code_offset;
	u8 packet_lost;
	u8 last_frameseq;
}_gma_ota_packet_hdl;
static _gma_ota_packet_hdl gma_ota_packet_hdl = {.data_buf.buffer = NULL};
static int gma_ota_buffer_manage_open(void)
{
	///free buffer
	if (gma_ota_packet_hdl.data_buf.buffer != NULL)
	{
		free(gma_ota_packet_hdl.data_buf.buffer);
		gma_ota_packet_hdl.data_buf.buffer = NULL;
	}
	if (gma_ota_packet_hdl.cmd_mg.buffer != NULL)
	{
		free(gma_ota_packet_hdl.cmd_mg.buffer);		
		gma_ota_packet_hdl.cmd_mg.buffer = NULL;
	}
	if (gma_ota_packet_hdl.cmd_mg.cmd_buf != NULL)
	{
		free(gma_ota_packet_hdl.cmd_mg.cmd_buf);		
		gma_ota_packet_hdl.cmd_mg.cmd_buf = NULL;
	}

	///data init
	memset(&gma_ota_packet_hdl,0x00,sizeof(_gma_ota_packet_hdl));

	///buffer init 
	if (gma_ota_packet_hdl.data_buf.buffer == NULL)
	{
		gma_ota_packet_hdl.data_buf.buffer = malloc((OTA_CIRCLE_PACKET_SIZE+1) * OAT_ONE_PACKET_MAX_SIZE);
		ASSERT(gma_ota_packet_hdl.data_buf.buffer);
	}
	if (gma_ota_packet_hdl.cmd_mg.buffer == NULL)
	{
		gma_ota_packet_hdl.cmd_mg.buffer = malloc(1024);		
		ASSERT(gma_ota_packet_hdl.cmd_mg.buffer);
		cbuf_init(&(gma_ota_packet_hdl.cmd_mg.cbuf),gma_ota_packet_hdl.cmd_mg.buffer,1024);
	}
	if (gma_ota_packet_hdl.cmd_mg.cmd_buf == NULL)
	{
		gma_ota_packet_hdl.cmd_mg.cmd_buf = malloc(1024);		
		ASSERT(gma_ota_packet_hdl.cmd_mg.cmd_buf);
	}
	gma_ota_packet_hdl.ota_buf_init = 1;

	return 0;
}

static int gma_ota_buffer_manage_close(void)
{

	gma_ota_packet_hdl.ota_buf_init = 0;
	if (gma_ota_packet_hdl.data_buf.buffer == NULL)
	{
		free(gma_ota_packet_hdl.data_buf.buffer);
		gma_ota_packet_hdl.data_buf.buffer = NULL;
...
void *gma_ota_cmd_get(u8 *buf,u16 len)
{
	if (gma_ota_packet_hdl.ota_buf_init == 0)
	{
		return (void *)(-1);
	}
	...
	if (len)
	{
		u32 wlen = cbuf_write(&(gma_ota_packet_hdl.cmd_mg.cbuf),buf,len);
		if (wlen !=  len)
		{
			GMA_DEBUG("gma cmd write overflow !!! \n");
		}
	}

    gma_frame_head_s gma_frame_head;
	u32 rlen = cbuf_read_alloc_len(&(gma_ota_packet_hdl.cmd_mg.cbuf),&gma_frame_head, sizeof(gma_frame_head_s));
	if (rlen != sizeof(gma_frame_head_s))
	{
		return (void *)(0);	
	}
	if ((sizeof(gma_frame_head_s) + gma_frame_head.f_len) > \
			cbuf_get_data_len(&(gma_ota_packet_hdl.cmd_mg.cbuf)))
	{
		return (void *)(0);	
	}

	cbuf_read(&(gma_ota_packet_hdl.cmd_mg.cbuf),gma_ota_packet_hdl.cmd_mg.cmd_buf,\
			(sizeof(gma_frame_head_s) + gma_frame_head.f_len));

	return gma_ota_packet_hdl.cmd_mg.cmd_buf;
}

static int gma_ota_buffer_manage_run(gma_recv_data_s *gma_recv_data)
{
#define SYNC_FRAME_SEQUENCE()	(gma_ota_packet_hdl.last_frameseq == gma_recv_data->gma_frame_head.total_fn)? \
	(gma_ota_packet_hdl.last_frameseq = 0) : (gma_ota_packet_hdl.last_frameseq = gma_recv_data->gma_frame_head.fn);

#define FRAMESQ_IS_LEGAL(cur_sq,last_sq)  (cur_sq == (last_sq + 1))

	if (gma_ota_packet_hdl.data_buf.buffer == NULL)
	{
		GMA_DEBUG("GMA OTA BUFFER ERROR !!! \n");
		return (-1);
	}

	GMA_DEBUG(" cur frame:%d total frame:%d \n",gma_recv_data->gma_frame_head.fn,gma_recv_data->gma_frame_head.total_fn);

	///sequence zero reset all information
	if (gma_recv_data->gma_frame_head.fn == 0)
	{
		gma_ota_packet_hdl.data_buf.buffer_offset = 0;
		gma_ota_packet_hdl.last_frameseq = 0;
		gma_ota_packet_hdl.packet_lost = 0;
	}
	
	///wait fn equal to zero when data had lost
	if (gma_ota_packet_hdl.packet_lost)
	{
		return (-1);
	}
	
	///frame sequence is ok
	if (gma_recv_data->gma_frame_head.fn && !FRAMESQ_IS_LEGAL(gma_recv_data->gma_frame_head.fn,gma_ota_packet_hdl.last_frameseq))
	{
		gma_ota_packet_hdl.packet_lost = 1;	
	}
	else
	{
		SYNC_FRAME_SEQUENCE();
	...
	if (gma_ota_packet_hdl.packet_lost)
	{
		///update to app have data lost
		gma_ota_data_request(gma_ota_packet_hdl.last_packet_code_offset);
		return (-1);
	}

	///data store
	if ((gma_ota_packet_hdl.data_buf.buffer_offset + gma_recv_data->gma_frame_head.f_len) > \
		((OTA_CIRCLE_PACKET_SIZE+1) * OAT_ONE_PACKET_MAX_SIZE))
	{
		GMA_DEBUG("frame overflow error !!!!:%d %d \n ",OTA_CIRCLE_PACKET_SIZE,gma_recv_data->gma_frame_head.total_fn);
		return (-1);
	}
	memcpy(gma_ota_packet_hdl.data_buf.buffer+gma_ota_packet_hdl.data_buf.buffer_offset,\
			gma_recv_data->recv_data,gma_recv_data->gma_frame_head.f_len);
	gma_ota_packet_hdl.data_buf.buffer_offset += gma_recv_data->gma_frame_head.f_len;

	///data ok
	(gma_recv_data->gma_frame_head.total_fn == gma_recv_data->gma_frame_head.fn)? \
			 gma_ota_packet_hdl.last_packet_code_offset += gma_ota_packet_hdl.data_buf.buffer_offset : (-1);

	return (gma_recv_data->gma_frame_head.total_fn == gma_recv_data->gma_frame_head.fn)? (0):(-1);
}
///check version
...
static sint32_t gma_reply_ota_update_para(uint8_t msg_id)
{
	...
	update_rply_para.fm_update_en? gma_ota_buffer_manage_open():0;
...
static sint32_t gma_recv_ota_req_para(gma_recv_data_s *gma_recv_data)
{
    ...
    put_buf(de_out, de_len);
extern uint8_t gma_ota_update_permit(void);
    if(gma_ota_update_permit()){
...
static sint32_t gma_ota_data_request(uint32_t last_frame_size)
{
    ...
    p_buf = (uint8_t *)(&reply_rsp_head);
    memcpy(send_buf, p_buf, sizeof(gma_frame_head_s));
    fm_frame_data_rply.totalFrame = OTA_CIRCLE_PACKET_SIZE;
...
static sint32_t gma_recv_ota_data(gma_recv_data_s *gma_recv_data)
{
	...
	gma_reply_ota_data(gma_recv_data->gma_frame_head.msg_id);

//	put_buf(gma_recv_data->recv_data,gma_recv_data->gma_frame_head.f_len);
//    return 0;
    if (gma_ota_buffer_manage_run(gma_recv_data) == 0) {
        gma_ota_update_data_cb(gma_ota_packet_hdl.data_buf.buffer,gma_ota_packet_hdl.data_buf.buffer_offset, gma_ota_data_request);
    }
...
static sint32_t gma_reply_ota_crc16(uint8_t msg_id,uint8_t crc16_res)
{
    if (!GMA_MODULE_IS_EN()) {
        GMA_DEBUG("gma module uninit !!! \n");
        return (-1);
...
static u16 ota_check_timeout_id = 0;
static u8 ota_check_cnt= 0;

//0  : 失败      1 : 成功
void gma_replay_ota_result(u8 result)
{
    if (tws_api_get_role() == TWS_ROLE_MASTER) {
	    gma_reply_ota_crc16(ota_data_msg_id,result);	
    }
}

static void check_ota_result(void *priv)
{
    u8 tws_ota_result[2]; 
	u8 crc16_res = 0; 
    ota_check_timeout_id = 0;
    //check ota result
    u8 result = tws_ota_control(OTA_RESULT_GET);
    if((result & BIT(TWS_ROLE_SLAVE)) && (result & BIT(TWS_ROLE_SLAVE))){
        g_printf("tws already ota succ1\n"); 
        crc16_res = 1; 
    }else{
        crc16_res = 0; 
        g_printf("tws ota err\n"); 
        ota_check_cnt++;
        /* if(ota_check_cnt > 30 || tws_ota_control(OTA_REMOTE_STATUS_GET) == OTA_VERIFY_END || tws_ota_control(OTA_REMOTE_STATUS_GET) == OTA_OVER){ */
        if(ota_check_cnt > 30){
        }else{
            ...
            ota_check_timeout_id = sys_timeout_add(NULL,check_ota_result,1000);
            return;
        }
    }


    //sync tws updata over , if tws all ota succ , update boot info
    /* tws_api_sync_call_by_uuid('T', SYNC_CMD_UPDATE_OVER, 50); */
    tws_ota_event_post(SYS_BT_OTA_EVENT_TYPE_STATUS,OTA_UPDATE_OVER);
    //if slave or master ota err , tell phone ota err
	/* gma_reply_ota_crc16(ota_data_msg_id,crc16_res);	 */
} 
...
sint32_t gma_reply_ota_crc_result(int crc_res)
{
    ...
    r_printf("gma_ota_master_result:%d %d\n",crc_res,tws_ota_control(OTA_STATUS_GET));

    //单耳升级
    if(tws_ota_control(OTA_TYPE_GET) == OTA_SINGLE_EARPHONE){
	    gma_reply_ota_crc16(ota_data_msg_id,crc_res);	
		dual_bank_update_burn_boot_info(dual_bank_update_burn_boot_info_callback);
        return 0;
    }

    //update ota result to master 
    tws_ota_control(OTA_RESULT_SET,crc_res,TWS_ROLE_MASTER);
    
    tws_ota_result[0] = tws_ota_control(OTA_RESULT_GET);
    tws_ota_result[1] = tws_ota_control(OTA_STATUS_GET);

    //send ota result to slave
    tws_ota_send_data_to_sibling(TWS_UPDATE_RESULT_EXCHANGE,(u8 *)&tws_ota_result,2);

    //check ota result
    u8 result = tws_ota_control(OTA_RESULT_GET);
    if((result & BIT(TWS_ROLE_SLAVE)) && (result & BIT(TWS_ROLE_SLAVE))){  //alraeady ota succ 
        g_printf("tws already ota succ 0\n"); 
        crc_res = 1; 
        ...
        if(ota_check_timeout_id){
            sys_timeout_del(ota_check_timeout_id); 
            ota_check_timeout_id = 0;
        }
        ota_check_cnt = 0;
        ota_check_timeout_id = sys_timeout_add(NULL,check_ota_result,500);
    }
...
static sint32_t gma_recv_ota_fm_code_check(gma_recv_data_s *gma_recv_data)
{
	...
	if(0 == gma_reply_ota_data(gma_recv_data->gma_frame_head.msg_id)) {
        if(tws_ota_control(OTA_TYPE_GET) == OTA_SINGLE_EARPHONE){
		    gma_ota_update_verify_cb(gma_reply_ota_crc_result);		
        }else{
            //tell slave already receive app ota data ok
            tws_ota_send_data_to_sibling(TWS_UPDATE_VERIFY,NULL,0);
        }
	}

    return 0;//gma_reply_ota_crc16();
}
...
sint32_t gma_heartbeat(void)
{
    ...
    reply_rsp_head.msg_id = 0;//gma_para_proc.next_msgid;
    reply_rsp_head.save_flag = 0;
    reply_rsp_head.version = 0;
    reply_rsp_head.cmd = COMMAND_HEARTBEAT;
    reply_rsp_head.fn = 0;
    reply_rsp_head.total_fn = 0;
...
/*
 *bt data buffer manage
 * */
struct __bt_buffer_mg
{
	cbuffer_t cbuf;
	u8 buffer[2*1024];
	u8 cmd_buf[1024];
};
static struct __bt_buffer_mg bt_buffer_mg;

void tm_cmd_analysis_resume(void);
sint32_t gma_cmd_analysis(uint8_t *packet, uint16_t len);
#define BT_BUFFER_MG_HDL (&(bt_buffer_mg.cbuf))
#define cbuf_get_total_size(a) (a)->total_len
#define cbuf_get_available_space_size(a) ((a)->total_len - (a)->data_len)
sint32_t gma_recv_proc(uint8_t *packet, uint16_t len)
{
	///check is there enough buffer
	if (cbuf_get_available_space_size(BT_BUFFER_MG_HDL) < (len + sizeof(uint16_t)))
	{
		GMA_DEBUG("gma cmd buffer no enouth space !!! \n");
		return 0;
	}

	if (len > sizeof(bt_buffer_mg.cmd_buf))
	{
		GMA_DEBUG("gma cmd too long !!! \n");
		return 0;
	}

    gma_hw_api.ENTER_CRITICAL();
	///write len
	cbuf_write(BT_BUFFER_MG_HDL,&len,sizeof(uint16_t));
	///write data 
	cbuf_write(BT_BUFFER_MG_HDL,packet,len);
    gma_hw_api.EXIT_CRITICAL();
	
	///kick gma process
	tm_cmd_analysis_resume();
	return 0;
}
...
{
	uint16_t len = 0;
	if (cbuf_get_data_len(BT_BUFFER_MG_HDL) == 0)
	{
		return;
	}
	///read len
	cbuf_read(BT_BUFFER_MG_HDL,&len,sizeof(uint16_t));
	///read dat
	cbuf_read(BT_BUFFER_MG_HDL,bt_buffer_mg.cmd_buf,len);

	gma_cmd_analysis(bt_buffer_mg.cmd_buf, len);
}
...
#if (GMA_OTA_EN)
	buf = gma_ota_cmd_get(packet,len);
	if (buf == (u8 *)(-1))
	{
		buf = packet;	
	}
...
_prase_packet:
...

    {
        ...
        GMA_DEBUG(">>>>gma_recv_data.gma_frame_head.cmd:%x \n", gma_recv_data.gma_frame_head.cmd);
proc_msg:
        gma_para_proc.next_msgid++;
        do {
 ...
 *  heartbeat
 */
static u16 gma_heartbeat_timer_hdl = 0;
static void gma_heartbeat_timer(void *priv)
{
    if (!GMA_MODULE_IS_EN()) {
        return ;
    }
...
void gma_init(int (*should_send)(_uint16 len), int(*send_cmd_data)(_uint8 *buf, _uint16 len), int(*send_audio_data)(_uint8 *buf, _uint16 len))
{
	...
	bt_check_exit_sniff();
	bt_check_enter_sniff();
	cbuf_init(&(bt_buffer_mg.cbuf),bt_buffer_mg.buffer,sizeof(bt_buffer_mg.buffer));
    gma_mac_addr_reverse_get(gma_para_proc.ble_mac);
	...
	(gma_heartbeat_timer_hdl == 0)? gma_heartbeat_timer_hdl = sys_timer_add(NULL, gma_heartbeat_timer,5 * 60000) : (0);
}
...
void gma_exit(void)
{
	...
	GMA_DEBUG("gma exit \n");
	(gma_heartbeat_timer_hdl != 0)? (sys_timer_del(gma_heartbeat_timer_hdl),gma_heartbeat_timer_hdl = 0) : (0);

#if (GMA_OTA_EN)
    //升级过程中退出手机app
    tws_ota_stop(OTA_STOP_APP_DISCONNECT);
#endif

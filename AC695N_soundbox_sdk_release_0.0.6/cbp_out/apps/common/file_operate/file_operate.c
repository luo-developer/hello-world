...
static struct file_storage_dev file_strg_nortone = {
};
static u32 nortone_time_stemp = 0;
#endif
...
REGISTER_STORAGE_DEVICES(stroage_dev) = {
    ...
    {"nor_tone", DEV_STR_HEAD"norflash", "storage/nor_tone", "storage/nor_tone/C/", "nor_sdfile", &file_strg_nortone, &nortone_time_stemp},
#endif
...
extern void audio_resume_all_mix(void);
...
// 有叠加的情况下激活一下解码
// 如果文件扫描所在task优先级较低或者不关心卡解码，可以不需要该处理
	static u32 check_tmr = 0;
	u32 cur_tmr = timer_get_ms() >> 2;
	if (check_tmr != cur_tmr) {
		check_tmr = cur_tmr;
		/* putchar('<'); */

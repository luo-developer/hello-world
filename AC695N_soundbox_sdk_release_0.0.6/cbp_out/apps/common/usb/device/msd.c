...
#else
#if USB_MSD_BULK_DEV_USE_ASYNC
u8 msd_buf[MSD_BUFFER_SIZE * 2] sec(.mass_storage) __attribute__((aligned(64)));
#else
u8 msd_buf[MSD_BUFFER_SIZE] sec(.mass_storage) __attribute__((aligned(64)));
#endif
struct usb_msd_handle _msd_handle sec(.mass_storage);
#endif
extern void clr_wdt();
...
u32 check_disk_status(void *dev_handle, u8 cur_lun)
{
    u32 err = 0;
    u32 online_status = 0;
    if (dev_handle != NULL) {
        err = dev_ioctl(dev_handle, IOCTL_GET_STATUS, (u32)(&online_status));
        if (online_status == 0) {
            log_error("online_status =%d, lun = %d", online_status, cur_lun);
            return RBC_SDFAIL;
        }
...
u32 msd_usb2ram(const struct usb_device_t *usb_device, u8 *buffer, u32 len)
{
    usb_dev usb_id = usb_device2id(usb_device);
    return usb_g_bulk_readram(usb_id, MSD_BULK_EP_OUT, buffer, len);
}
...
static void test_unit_ready(const struct usb_device_t *usb_device)
{
    ...
    if (RBC_SDFAIL == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
...
static void msd_write_10_async(const struct usb_device_t *usb_device, u8 cur_lun, u32 lba, u16 lba_num)
{
    ...
    while (lba_num) {
        ...
        err = msd_usb2ram(usb_device, msd_handle->msd_buf + buf_idx * MSD_BUFFER_SIZE, num * 0x200);
        if (err != num * 0x200) {
            log_error("read usb_err %d, dev = %s\n", __LINE__, msd_handle->info.dev_name[cur_lun]);
            stall_error(usb_device, 1, MEDIUM_ERROR);
            break;
        }
        if (RBC_SDPASS == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
            dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_SET_ASYNC_MODE, 0);
            err = dev_bulk_write(msd_handle->info.dev_handle[cur_lun], msd_handle->msd_buf + buf_idx * MSD_BUFFER_SIZE, lba, num);
            if (err != num) {
    ...
    if (RBC_SDPASS == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
        ...
        dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_FLUSH, 0);
    } else {
...
static void msd_read_10_async(const struct usb_device_t *usb_device, u8 cur_lun, u32 lba, u16 lba_num)
{
    ...
    if (RBC_SDPASS == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
        dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_SET_ASYNC_MODE, 0);
        err = dev_bulk_read(msd_handle->info.dev_handle[cur_lun], msd_handle->msd_buf + buf_idx * MSD_BUFFER_SIZE, lba, num);
        if (err != num) {
...
static void write_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        err = msd_usb2ram(usb_device, msd_handle->msd_buf, num * 0x200);
        if (err == 0) {
            stall_error(usb_device, 1, MEDIUM_ERROR);
        ...
        if (RBC_SDPASS == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
            dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_CMD_RESUME, 0);
            err = dev_bulk_write(msd_handle->info.dev_handle[cur_lun], msd_handle->msd_buf, lba, num);
            dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_CMD_SUSPEND, 0);
            if (err != num) {
                stall_error(usb_device, 1, MEDIUM_ERROR);
...
static void read_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        if (RBC_SDPASS == check_disk_status(msd_handle->info.dev_handle[cur_lun], cur_lun)) {
            dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_CMD_RESUME, 0);
            err = dev_bulk_read(msd_handle->info.dev_handle[cur_lun], msd_handle->msd_buf, lba, num);
            dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_CMD_SUSPEND, 0);
            if (err != num) {
...
int offset_rec = 0x40000; //set by customer
static void read_capacity(const struct usb_device_t *usb_device)
{
    ...
    dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_GET_CAPACITY, (u32)&capacity_temp);
#if (TCFG_NOR_FS_ENABLE && TCFG_NORFLASH_DEV_ENABLE)
    if (!strcmp(msd_handle->info.dev_name[cur_lun], "norflash")) {
        /* r_printf(">>>[test]:capacity_temp1111 = %d\n", capacity_temp); */
        extern int offset_addr; 
        capacity_temp = (1024*1024) / 512;
        offset_addr = (1024 * 1024) + offset_rec + 4096;
        /* capacity_temp = capacity_temp / 2; */
        /* offset_addr = capacity_temp * 512; */
    }
#endif
    capacity_temp = cpu_to_be32(capacity_temp - 1);
    memcpy(capacity, &capacity_temp, 4);
    dev_ioctl(msd_handle->info.dev_handle[cur_lun], IOCTL_GET_BLOCK_SIZE, (u32)&capacity_temp);
...
#if (TCFG_NOR_FS_ENABLE && TCFG_NORFLASH_DEV_ENABLE)
    if (!strcmp(msd_handle->info.dev_name[cur_lun], "norflash")) {
        /* r_printf(">>>[test]:capacity_temp222 = %d\n", capacity_temp); */
        extern int offset_addr; 
        capacity_temp = (1024*1024) / 512;
        offset_addr = (1024 * 1024) + offset_rec + 4096;
        /* capacity_temp = capacity_temp / 2; */
...
u32 msd_register_disk(const char *name, void *arg)
{
    ...
    if (msd_handle) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (msd_handle->info.dev_handle[i] == NULL) {
                msd_handle->info.dev_handle[i] = dev_open(name, arg);
                if (msd_handle->info.dev_handle[i]) {
                    len = MSD_DEV_NAME_LEN - 1 > strlen(name) ? strlen(name) : MSD_DEV_NAME_LEN - 1;
                    strncpy(msd_handle->info.dev_name[i], name, len);
                    recover_set_cardreader_popup(i);
                    ret = 0;
                } else {
                    ret = -2;
                }

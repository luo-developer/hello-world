...
void *sin_tone_open(const struct sin_param *param, int num, u8 channel, u8 repeat)
{
    ...
    sin_maker_handle.sin = param;
    sin_maker_handle.sin_num = num;
    sin_maker_handle.open = 1;
    sin_maker_handle.points = param[0].points;
    sin_maker_handle.id = 0;
    sin_maker_handle.channel = channel;
    sin_maker_handle.repeat = repeat;
#if (defined CONFIG_CPU_BR18 || \
     CONFIG_CPU_BR21)
    /*这里如果加多fade points，会导致结束的时候有噪声,不是在淡出尾部*/
    sin_maker_handle.fade_points = 0;
#else
    sin_maker_handle.fade_points = 480;
#endif
    sin_maker_handle.next_param = 1;

    return (void *)&sin_maker_handle;
}

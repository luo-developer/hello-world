...
#define TONE_SYNC_BY_BT_TIME_EN		1
#else
#define TONE_SYNC_BY_BT_TIME_EN		0
#endif
...
#if TONE_SYNC_BY_BT_TIME_EN
extern void tone_out_start_by_bt_time(u32 bt_time);
extern u32 audio_bt_time_read(u16 *bt_phase, s32 *bt_bitoff);

struct tone_sync_tws {
    u32 tmr;
};

static void tone_sync_tws_func(void *_data, u16 len, bool rx)
{
    struct tone_sync_tws tws;
    if (rx) {
...
REGISTER_TWS_FUNC_STUB(tone_sync_stub) = {
    .func_id = TWS_FUNC_ID_TONE_SYNC,
    .func    = tone_sync_tws_func,
};
#endif /* TONE_SYNC_BY_BT_TIME_EN */

int bt_tws_api_push_cmd(int priv, int delay_ms)
{
#if TONE_SYNC_BY_BT_TIME_EN
	if ((priv == SYNC_CMD_PHONE_CONN_TONE)
	 || (priv == SYNC_CMD_LED_PHONE_DISCONN_STATUS)
	   ) {
		int delay_tmr = 300;
		if (app_cur_task_check(APP_NAME_BT) == false) {
			delay_tmr = 1500;
		}
		struct tone_sync_tws tws = {0};
		tws.tmr = audio_bt_time_read(NULL, NULL) + (delay_ms+delay_tmr)*1000/625;
		tone_out_start_by_bt_time(tws.tmr);
		printf("bt_time:%d,\n", tws.tmr);
		int err = tws_api_send_data_to_sibling((u8 *)&tws, sizeof(struct tone_sync_tws), TWS_FUNC_ID_TONE_SYNC);
        return tws_api_sync_call_by_uuid('T', priv, delay_ms/2);
	}
#endif
...
void bt_tws_wait_pair_all_way(void)
{
...
static void bt_tws_reverb_state_sync(void *_data, u16 len, bool rx)
{
    if (rx) {
        ...
        r_f_printf("----rx  bt_tws_reverb_state_sync %d \n", data[0]);
    }
...
void bt_tws_sync_reverb_state(u8 status)
{
    ...
    r_f_printf("---- tx  bt_tws_sync_reverb_state %d \n", data[0]);
...
extern u32 audio_bt_time_read(u16 *bt_phase, s32 *bt_bitoff);
extern u32 a2dp_dec_output_set_start_bt_time(u32 bt_tmr);

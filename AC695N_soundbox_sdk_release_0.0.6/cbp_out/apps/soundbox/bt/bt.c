...
extern int bt_tws_api_push_cmd(int priv, int delay_ms);
...
void bt_reverb_status_change(struct bt_event *bt)
{
    ...
    r_f_printf("BT_STATUS_TWS_REVERB_CHANGE:%d\n",bt->value);
    if (bt->value == 1) {
        reverb_pause();
    } else {
        reverb_resume();
...
#define AVC_VOLUME_DOWN			0x42
    ...
    case  BT_STATUS_RECONN_OR_CONN:
        log_info("  BT_STATUS_RECONN_OR_CONN %d\n",bt->value);
		break;
    default:

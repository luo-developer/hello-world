...
#include "audio_mic/reverb/reverb.h"

#if (defined(SOUNDCARD_ENABLE) && SOUNDCARD_ENABLE)
...
void soundcard_user_msg_deal(int msg, int argc, int *argv)
{
	switch(msg)
	{
		case USER_MSG_SYS_SOUNDCARD_SLIDE_MIC:
			__this->mic_vol = argv[0];//get_max_sys_vol() * argv[0] / 10;;
			y_printf("mic vol %d \n",__this->mic_vol);
			if(__this->mic_online){
				reverb_set_dvol(__this->mic_vol);
				/* reverb_user_digital_volume_set(__this->mic_vol); */
			}
			break;
		case USER_MSG_SYS_SOUNDCARD_SLIDE_ECHO:
			...
			reverb_set_wet(__this->echo_vol);
			break;
...
void soundcard_key_event_deal(u8 key_event)
{
	...
	switch(key_event)
	{
		...
		case  KEY_USB_MIC_CH_SWITCH:
			...
			{
				u32 mask = reverb_get_function_mask();
				printf("mask = %x\n", mask);
				static u8 tst_flag = 0;
				if(tst_flag)
				{
					reverb_set_function_mask(0);	
				}
				else
				{
					reverb_set_function_mask((u32)-1);	
				}
				tst_flag = !tst_flag;
				break;

				if(reverb_get_status())
				{
					reverb_stop();	
				}
				else

...
extern void audio_dec_bt_emitter_check_empty_en(u8 en);
...
static void tone_file_dec_release()
{
    if (file_dec) {
        ...
        local_irq_disable();
        if (file_dec->quad_data) {
            free(file_dec->quad_data);
            file_dec->quad_data = NULL;
        }
        free(file_dec);
        file_dec = NULL;
        local_irq_enable();
...
static int tone_fread(struct audio_decoder *decoder, void *buf, u32 len)
{
    ...
    if (bt_user_priv_var.emitter_or_receiver == BT_EMITTER_EN) {
        if (audio_sbc_enc_is_work() == false) {
            return 0;
        }
    }
#endif
...
#else
    wlen = audio_dual_to_quad_output_src(dec->hw_src, &dec->mix_ch, data, len, TONE_DEC_ID, dec->quad_data, NULL);
    len -= wlen;
    if (len == 0) {
        dec->remain = 0;
    } else {
...
#define TONE_OUT_BEFORE_BT_TIME			(100)
#define TONE_OUT_DELAY_BT_TIME			(200)
...
static void tone_out_lock_timer(void *priv)
{
    ...
    if (cur_tmr >= p_tone_lock_hdl->bt_time - TONE_OUT_BEFORE_BT_TIME) {
        /* putchar('&'); */
...
static int tone_dec_output_by_bt_time_handler(struct audio_decoder *decoder, s16 *data, int len, void *priv)
{
    ...
    if (p_tone_lock_hdl->tmr) {
...
#if TONE_OUT_BEFORE_BT_TIME
        if (p_tone_lock_hdl->before_len == 0) {
            p_tone_lock_hdl->before_len = TONE_OUT_BEFORE_BT_TIME * 625 * decoder->fmt.sample_rate / 1000000;
            p_tone_lock_hdl->before_len *= 2;
            if (!AUDIO_CH_IS_MONO(dec->channel)) {
                p_tone_lock_hdl->before_len *= 2;
            }
            printf("before_len:%d \n", p_tone_lock_hdl->before_len);
        }

        if (p_tone_lock_hdl->before_len) {
            // 填一些数据，保证两边的dac都已经打开
            u16 wlen;
            s16 tmp_buf[16];
            memset(tmp_buf, 0, sizeof(tmp_buf));
            do {
                wlen = p_tone_lock_hdl->before_len;
                ...
                p_tone_lock_hdl->before_len -= wlen;
            } while (p_tone_lock_hdl->before_len);
        }
#endif
        u16 phase = 0;
        s32 bitoff = 0;
        ...
        u32 cur_out_len = app_audio_output_get_cur_buf_points();
        local_irq_enable();
        ...
        if (offset_us < ((int)TONE_OUT_DELAY_TIME_US * (-1))) {
...
#if 1
            p_tone_lock_hdl->empty_len = (TONE_OUT_DELAY_TIME_US - offset_us) * decoder->fmt.sample_rate / 1000000;
            ...
            while (cnt--) {
                ...
                cur_out_len = app_audio_output_get_cur_buf_points();
                local_irq_enable();
        ...
        p_tone_lock_hdl->empty_len += (1 * 1024);
        u8 ch_num = audio_output_channel_num();
        u32 mixer_remain_len = (mixer.remain_points + dec->mix_ch.offset) / ch_num;
        u32 olen = (mixer_remain_len + cur_out_len) * decoder->fmt.sample_rate / dec->src_out_sr;
        if (p_tone_lock_hdl->empty_len > olen) {
            p_tone_lock_hdl->empty_len -= olen;
        } else {
            p_tone_lock_hdl->empty_len = 0;
        }
        p_tone_lock_hdl->empty_len *= 2;
        ...
        printf("chnum:%d, dec_cl:%d, mixer_remain_len:%d, cur_out_len:%d, olen:%d, empty:%d \n",
               ch_num, dec->channel,
               mixer_remain_len, cur_out_len, olen,
               p_tone_lock_hdl->empty_len);
...
int tone_dec_start()
{
    ...
    /*
        if (fmt->coding_type == AUDIO_CODING_MSBC) {
    ...
    */
    ...
    audio_mixer_ch_set_ext_out_mask(&file_dec->mix_ch, BIT(0));
    ...
    if (fmt->sample_rate != file_dec->src_out_sr) {
        ...
        file_dec->hw_src = audio_hw_resample_open(&file_dec->decoder, tone_dec_src_output_handler,
                           ch_num, fmt->sample_rate, file_dec->src_out_sr);
    }
...
#else
    if (!file_dec->quad_data) {
        file_dec->quad_data = (s16 *)malloc(DUAL_TO_QUAD_POINTS * 2 * 2); //四通道buf
    }
#endif
#endif
...
static int sine_dec_probe_handler(struct audio_decoder *decoder)
{
    ...
    if (!sine_dec->sin_maker) {
...
#endif
#else
        int channel     = audio_output_channel_num();//app_audio_output_mode_get() == DAC_OUTPUT_LR ? 2 : 1;
#endif
        ...
        if (!sample_rate) {
            sample_rate = audio_output_rate(app_audio_output_samplerate_get());
        }
...
int tone_play_stop_by_index(u8 index)
{
    ...
    if (!tone_dec || !tone_dec->list[0]) {
        return -2;
    }
    y_printf("tone, lst:0x%x,%s, idx:0x%x,%s \n", (int)*tone_dec->list[0], *tone_dec->list[0], (int)tone_index[index], tone_index[index]);
    if ((int)*tone_dec->list[0] == (int)tone_index[index]) {
        tone_stop(END_ABNORMAL);
    }

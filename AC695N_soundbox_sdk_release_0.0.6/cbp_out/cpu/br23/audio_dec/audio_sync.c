...
void *a2dp_play_sync_open(struct audio_decoder *dec, u8 channel, u32 sample_rate, u32 output_rate, u32 coding_type)
{
    ...
    sync_param.dev = NULL;
    audio_sync_hdl = audio_wireless_sync_open(&sync_param);
#elif (AUDIO_OUTPUT_ONLY_DAC)
    sync_param.target = AUDIO_SYNC_TARGET_DAC;
    sync_param.dev = &dac_hdl;
    audio_sync_hdl = audio_wireless_sync_open(&sync_param);
#elif (AUDIO_OUTPUT_ONLY_IIS)
    sync_param.target = AUDIO_SYNC_TARGET_IIS;
    sync_param.dev = &iis_hdl;
    sync_param.output_rate = TCFG_IIS_OUTPUT_SR;
    audio_sync_hdl = audio_wireless_sync_open(&sync_param);
    ...
    sync_param.target = AUDIO_SYNC_TARGET_IIS;
    if (audio_sync_hdl) {
        audio_wireless_sync_add_dev(audio_sync_hdl, &sync_param);
    } else {
        audio_sync_hdl = audio_wireless_sync_open(&sync_param);
    }
#endif

    return audio_sync_hdl;
}

...
extern struct audio_decoder_task decode_task_local;
#define LOCALTWS_TASK_HDL		decode_task_local
#else
#define LOCALTWS_TASK_HDL		decode_task
#endif

#if (defined(TCFG_DEC2TWS_ENABLE) && (TCFG_DEC2TWS_ENABLE))

#if (defined(DEC_MIX_ENABLE) && (DEC_MIX_ENABLE))
#define LOCALTWS_RESET_BY_BT_TIME_EN		1
#else
#define LOCALTWS_RESET_BY_BT_TIME_EN		0
#endif

u8 is_tws_active_device(void);

#define DEC2TWS_MEDIA_ALLOC_EN		0	// 使用空间申请方式
...
#undef LOCAL_TWS_EQ_SUPPORT_32BIT
#define LOCAL_TWS_EQ_SUPPORT_32BIT		0
#endif


#define AUDIO_LOCAL_TWS_VOL_FADE_EN					0 // 淡入
#define AUDIO_LOCAL_TWS_VOL_FADE_CNT_TIME_MS		(50)	// 淡入时长
...
void local_tws_fade_run(struct local_tws_dec_hdl *dec, s16 *buf, u16 len)
{
    ...
    for (u16 i = 0; i < len / 2; i++) {
		...
		if (valuetemp) {
			...
			if ((dec->fade_cnt == 0) && (dec->fade_idx == 0)) {
				continue;
			}
		}
        dec->fade_cnt += 2;
...
static void local_tws_mixer_event_handler(void *priv, int event)
{
    switch (event) {
...
static void *local_tws_audio_sync_open(struct local_tws_dec_hdl *dec, int sample_rate, int out_sample_rate)
{
    ...
    if (!local_tws_audio_sync) {
        /*音频同步声道配置为最后一级输出的声道数*/
        local_tws_audio_sync = local_tws_play_sync_open(&dec->decoder, channel, sample_rate, out_sample_rate);
    }

    if (local_tws_audio_sync) {
        audio_mixer_ch_set_event_handler(&dec->mix_ch, dec, local_tws_mixer_event_handler);
...
{
    os_mutex_create(&g_dec2tws.mutex);
#if DEC2TWS_MEDIA_ALLOC_EN
...
static void tws2dec_timeout_func(void *priv)
{
    ...
    if (g_dec2tws.media_value && (!is_tws_active_device())) {
...
#if AUDIO_LOCAL_TWS_SUPPORT_SYNC
            if (local_tws_audio_sync && dec->dec_out_len) {
                audio_wireless_sync_after_dec(local_tws_audio_sync, NULL, dec->dec_out_len);
            }
#endif
            dec->dec_out_len = 0;
        }
...
#if LOCALTWS_RESET_BY_BT_TIME_EN
extern u32 audio_bt_time_read(u16 *bt_phase, s32 *bt_bitoff);

static u32 localtws_bt_tmr = 0;
void localtws_dec_output_set_start_bt_time(u32 bt_tmr)
{
    localtws_bt_tmr = bt_tmr;
    y_printf("localtws_dec_output_set_start_bt_time:%d \n", bt_tmr);
}
void localtws_dec_output_set_reset_ms(u32 ms)
{
    localtws_dec_output_set_start_bt_time(audio_bt_time_read(NULL, NULL) + ms * 1000 / 625);
}
int localtws_dec_output_run_by_start_bt_time(struct audio_decoder *decoder, s16 *data, int len)
{
    if (localtws_bt_tmr == 0) {
        return -1;
    }
    u16 phase = 0;
    s32 bitoff = 0;
    u32 cur_tmr = audio_bt_time_read(&phase, &bitoff);
    int offset = (localtws_bt_tmr - cur_tmr) * 625 + phase - bitoff;
    /* y_printf("localtws_dec_output_run_by_start_bt_time, cur:%d, bt:%d, offset:%d \n", cur_tmr, localtws_bt_tmr, offset); */
    if (offset <= 0) {
        localtws_bt_tmr = 0;
        return -2;
...
void localtws_dec_output_reset(u32  ms)
{
#if LOCALTWS_RESET_BY_BT_TIME_EN
    localtws_dec_output_set_reset_ms(ms);
#else
...
int local_tws_eq_32bit_to_16bit(struct local_tws_dec_hdl *dec, s32 *data, u32 len)
{
...
static int local_tws_eq_output(void *priv, s16 *data, u32 len)
{
	...
	return local_tws_eq_32bit_to_16bit(dec, data, len);
#endif /*LOCAL_TWS_EQ_SUPPORT_32BIT*/
...
static int local_tws_dec_stop_and_restart(struct audio_decoder *decoder)
{
    ...
    if (local_tws_audio_sync) {
        ...
        local_tws_audio_sync_open(dec, dec->sample_rate, dec->src_out_sr);
#endif
    }
    return 0;
}
...
static int local_tws_dec_probe_handler(struct audio_decoder *decoder)
{
    ...
    if (0) { ; }
        ...
        if (!pkt_addr || (pkt_len < DEC2TWS_MEDIA_FMT_LEN)) {
            ...
            /* if (audio_decoder_running_number(&LOCALTWS_TASK_HDL) <= 1) { */
            /* os_time_dly(2); */
            /* } */
            return -EAGAIN;
        }
        if (pkt_addr[0] == DEC2TWS_MEDIA_INFO_PAUSE) {
            ...
            local_tws_audio_sync = NULL;
            dec->sync_start = 0;
            local_tws_audio_sync_open(dec, dec->sample_rate, audio_output_rate(dec->sample_rate));
        ...
        if (err) {
            ...
            if (err == SYNC_ERR_STREAM_RESET) {
                audio_mixer_ch_pause(&dec->mix_ch, 1);
                dec->read_en = 0;
                if (g_dec2tws.tws_send_pause || g_dec2tws.tws_stop) {
...
#if LOCALTWS_RESET_BY_BT_TIME_EN
        localtws_dec_output_run_by_start_bt_time(decoder, data, len);
#endif
...
{
    if (tws_api_local_media_trans_check_total(1) <= (DEC2TWS_MEDIA_BUF_LEN * 9 / 10)) {
        /* putchar('A'); */
        /* dec2tws_master_dec_resume(); */
        int limit_dec[1];
...
static int local_tws_dec_stop_handler(struct audio_decoder *decoder)
{
    struct local_tws_dec_hdl *dec = container_of(decoder, struct local_tws_dec_hdl, decoder);
#if AUDIO_LOCAL_TWS_SUPPORT_SYNC
    if (local_tws_audio_sync) {
...
static const struct audio_dec_handler local_tws_dec_handler = {
    ...
    .dec_stop 	= local_tws_dec_stop_handler,
};
...
static void local_tws_dec_release()
{
    ...
    audio_decoder_task_del_wait(&LOCALTWS_TASK_HDL, &local_tws_dec->wait);
...
static void local_tws_dec_event_handler(struct audio_decoder *decoder, int argc, int *argv)
{
    ...
    case AUDIO_DEC_EVENT_END:
        ...
        //audio_decoder_resume_all(&LOCALTWS_TASK_HDL);
        break;
    }
}
...
static int local_tws_dec_start()
{
    ...
    if (dec->dec_type & AUDIO_CODING_FAME_MASK) {
        dec->need_get_fmt = 1;
        err = audio_decoder_open(&dec->decoder, &local_fame_tws_input, &LOCALTWS_TASK_HDL);
    } else {
        err = audio_decoder_open(&dec->decoder, &local_file_tws_input, &LOCALTWS_TASK_HDL);
    }
#else
...
#if LOCAL_TWS_EQ_SUPPORT_32BIT
		audio_eq_set_info(dec->p_eq, local_tws_eq_param.channels, 1);
#endif
    ...
    local_tws_audio_sync_open(dec, dec->src_out_sr, dec->src_out_sr);
#endif
...
int local_tws_dec_open(u32 value)
{
    ...
    os_mutex_pend(&g_dec2tws.mutex, 0);
    if (!local_tws_dec) {
        local_tws_dec_open_mem();
    } else {
        os_mutex_post(&g_dec2tws.mutex);
        return 0;
    }
    ...
    err = audio_decoder_task_add_wait(&LOCALTWS_TASK_HDL, &dec->wait);
...
#if AUDIO_LOCAL_TWS_SUPPORT_SYNC
    if (local_tws_dec->sync_start) {
        return true;
    }
#else
    if (local_tws_dec->read_en) {
        return true;
    }
#endif
    return false;
...
int encfile_tws_dec_open(struct audio_fmt *pfmt)
{
    ...
    err = audio_decoder_open(&dec->decoder, &encfile_tws_input, &LOCALTWS_TASK_HDL);
    if (err) {
        goto __err1;
    }

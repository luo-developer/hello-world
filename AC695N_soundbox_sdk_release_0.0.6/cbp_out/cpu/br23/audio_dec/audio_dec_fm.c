...
static u8 audio_fm_dec_get_volume_defualt(u8 state)
{
	return app_audio_get_volume(APP_AUDIO_CURRENT_STATE);	
}

REGISTER_DEC_BOARD_PARAM(fm_dec_param) = 
{
	.name = "fm_dec_param",
	.d_vol_en = 1,		
	.d_vol_max = (void *)app_audio_get_max_volume,		
	.d_vol_cur = (void *)audio_fm_dec_get_volume_defualt,
	.d_vol_fade_step = 2,		
	.voice_cut_en  = 0,
};
...
void fm_dec_relaese()
{
    ...
    if (fm_dec) {
        ...
        local_irq_disable();
        if (fm_dec->quad_data) {
            free(fm_dec->quad_data);
            fm_dec->quad_data = NULL;
        }
    ...
    if (!dec->sync_start) {
		...
		if (dec->dec_no_out_sound) {
			...
			if ((dec->tws_out_flag != tws_out_flag) || (tws_out_flag == false)) {
				...
				while (linein_sample_read(dec->fm, (void *)buf, len)) {
					;
				}
				memset(buf, 0, len);
				return len;
			}
			if (linein_sample_size(dec->fm) > len) {
				/* putchar('S'); */
				dec->sync_start = 1;
				return -1;
			}
...
static int fm_eq_output2(void *priv, s16 *data, u32 len)
{
    return len;
}
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
#if (defined(TCFG_EQ_DIVIDE_ENABLE) && (TCFG_EQ_DIVIDE_ENABLE !=0 ))
int fm_eq_get_source(void *priv, s16 *buf, len)
{
...
static int fm_eq_output_async(void *priv, s16 *data, u32 len)
{
    int wlen = 0;
    int total_wlen = 0;
    int rlen = len;
...
int fm_eq_32bit_to_16bit(struct fm_dec_hdl *dec, s32 *data, u32 len)
{
...
static int fm_eq_output(void *priv, s16 *data, u32 len)
{
	...
	return fm_eq_32bit_to_16bit(dec, data, len);
#endif /*FM_EQ_SUPPORT_32BIT*/
...
#else
    wlen = audio_dualeq_to_quadeq_output_sync(dec->src_sync, eq, drc, &dec->mix_ch, data, len, FM_DEC_ID, dec->quad_data, NULL);
#endif
#else
    wlen = audio_dual_to_quad_output_src(dec->src_sync, &dec->mix_ch, data, len, FM_DEC_ID, dec->quad_data, NULL);
#endif
    len -= wlen;
    if (len == 0) {
        dec->remain = 0;
...
int fm_dec_start()
{
    ...
    if (!dec->quad_data) {
        printf("fm quad_data malloc err\n");
    }
#endif
#endif
	...
	vol_parm.en  = fm_dec_param.d_vol_en;
	vol_parm.vol_max = ((fm_dec_param.d_vol_max != NULL) ? fm_dec_param.d_vol_max() : 0);
	vol_parm.vol = ((fm_dec_param.d_vol_cur != NULL) ? fm_dec_param.d_vol_cur() : 0);
    vol_parm.fade_step = fm_dec_param.d_vol_fade_step;
#endif
    dec->user_hdl = user_audio_process_open((void *)&vol_parm, (void *)&fm_dec_param, user_audio_process_handler);
#endif
...
#else
/* #if FM_EQ_SUPPORT_ASYNC */
        audio_eq_set_output_handle(dec->p_eq, fm_eq_output_async, dec);
#if (defined(TCFG_EQ_DIVIDE_ENABLE) && (TCFG_EQ_DIVIDE_ENABLE !=0 ))
        audio_eq_set_output_source_handle(dec->p_eq, fm_eq_get_source, dec);
        dec->rr_rl_source = (s16 *)&dec->quad_data[DUAL_TO_QUAD_POINTS];
#endif
/* #else */
        /* audio_eq_set_output_handle(dec->p_eq, fm_eq_output2, dec); */
/* #endif */
#endif
#else
        audio_eq_set_output_handle(dec->p_eq, fm_eq_output, dec);
    ...
    if (dec->p_eq2) {
        ...
        audio_eq_set_output_handle(dec->p_eq2, fm_eq_output2, dec);

...
struct file_dec_hdl {
    ...
    /* PITCHSHIFT_FUNC_API *audio_effect_ops; */
    /* void *audio_effect_ops_buf; */
    /* short*effect_outdata; */
    /* RMONO2STEREO_FUNC_API *audio_mono2stereo_ops; */
    /* void *audio_mono2stereo_ops_buf; */
    /* s16 signal_buf[512]; */
#endif
...
static u8 audio_file_dec_get_volume_defualt(u8 state)
{
	return app_audio_get_volume(APP_AUDIO_CURRENT_STATE);	
}

REGISTER_DEC_BOARD_PARAM(file_dec_param) = 
{
	.name = "file_dec_param",
	.d_vol_en = 1,		
	.d_vol_max = (void *)app_audio_get_max_volume,		
	.d_vol_cur = (void *)audio_file_dec_get_volume_defualt,
	.d_vol_fade_step = 2,		
	.voice_cut_en  = 0,
};
...
int music_eq_32bit_to_16bit(struct file_dec_hdl *dec, s32 *data, u32 len)
{
...
#if MUSIC_EQ_SUPPORT_32BIT
		return music_eq_32bit_to_16bit(dec, data, len);
#endif /*MUSIC_EQ_SUPPORT_32BIT*/
...
#if (defined(TCFG_EQ_DIVIDE_ENABLE) && (TCFG_EQ_DIVIDE_ENABLE !=0 ))
int file_eq_get_source(void *priv, s16 *buf, len)
{
    struct file_dec_hdl *dec = priv;
...
#else
    wlen = audio_dualeq_to_quadeq_output_sync(dec->hw_src, eq, drc, &dec->mix_ch, data, len, FILE_DEC_ID, dec->quad_data, NULL);
#endif
#else
    wlen = audio_dual_to_quad_output_src(dec->hw_src, &dec->mix_ch, data, len, FILE_DEC_ID, dec->quad_data, NULL);
...
static int file_dec_start()
{
    ...
    if (pcm_enc_flag && (dec->decoder.dec_ops->coding_type & FILE_DEC_PCM_DEC_TYPE_MASK)) {
        ...
        enc_fmt.sample_rate = audio_output_rate(dec->decoder.fmt.sample_rate);//dec->decoder.fmt.sample_rate;
#if (defined(TCFG_PCM2TWS_SBC_ENABLE) && (TCFG_PCM2TWS_SBC_ENABLE))
	...
	vol_parm.en  = file_dec_param.d_vol_en;
	vol_parm.vol_max = ((file_dec_param.d_vol_max != NULL) ? file_dec_param.d_vol_max() : 0);
	vol_parm.vol = ((file_dec_param.d_vol_cur != NULL) ? file_dec_param.d_vol_cur() : 0);
    vol_parm.fade_step = file_dec_param.d_vol_fade_step;
#endif
    dec->user_hdl = user_audio_process_open((void *)&vol_parm, (void *)&file_dec_param, user_audio_process_handler);
#endif
...
__dec_start:
    ...
    if ((fmt->sample_rate != dec->src_out_sr) && (!pick_flag)) {
        u8 ch_num = dec->ch_num;
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
...
#else
/* #if MUSIC_EQ_SUPPORT_ASYNC */
        audio_eq_set_output_handle(dec->p_eq, file_eq_output_async, dec);

#if (defined(TCFG_EQ_DIVIDE_ENABLE) && (TCFG_EQ_DIVIDE_ENABLE !=0 ))
        audio_eq_set_output_source_handle(dec->p_eq, file_eq_get_source, dec);
        dec->rr_rl_source = (s16 *)&dec->quad_data[DUAL_TO_QUAD_POINTS];

#endif
/* #else */
        /* audio_eq_set_output_handle(dec->p_eq, file_eq_output2, dec); */
/* #endif */
#endif
#else
        audio_eq_set_output_handle(dec->p_eq, file_eq_output, dec);
    ...
    if (dec->p_eq2) {
        ...
        file_eq_param.remain_en = 1;
        file_eq_param.max_nsection = EQ_SECTION_MAX;
        file_eq_param.cb = eq_get_filter_info2;
        file_eq_param.no_wait = 0;//同步
        file_eq_param.eq_name = 1;
        audio_eq_open(dec->p_eq2, &file_eq_param);
        audio_eq_set_samplerate(dec->p_eq2, fmt->sample_rate);
        audio_eq_set_output_handle(dec->p_eq2, file_eq_output2, dec);

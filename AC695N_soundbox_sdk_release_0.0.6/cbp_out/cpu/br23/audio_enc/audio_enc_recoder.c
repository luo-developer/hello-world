...
#if (defined(THREE_ADC_ENABLE) && (TCFG_REVERB_SAMPLERATE_DEFUALT >= 32000))
#define LADC_IRQ_POINTS     128//48
#else
...
int record_player_encode_start(struct audio_fmt *f, struct storage_dev *dev, enum enc_source source, u32 cut_head_time, u32 cut_tail_time, u32 limit_size)
{
    ...
    if (f) {
		...
		fmt.coding_type = AUDIO_CODING_WAV;//AUDIO_CODING_G726;;//AUDIO_CODING_MP3;//
#if TCFG_NOR_FS_ENABLE
		fmt.coding_type = AUDIO_CODING_WAV;//nor_fs only support wav now!!!
#endif
    ...
    if ((fmt.coding_type == AUDIO_CODING_WAV) || (fmt.coding_type == AUDIO_CODING_G726)) {
		...
		cut_tail_size = fmt.sample_rate * fmt.channel * cut_tail_time * 2 / 1000;
    } else {
        cut_tail_size = fmt.bit_rate * cut_tail_time / 8;
    }

    if ((fmt.coding_type == AUDIO_CODING_WAV) || ((fmt.coding_type == AUDIO_CODING_PCM))) {
    ...
    case ENCODE_SOURCE_MIC:
        ...
        audio_adc_mic_set_gain(&rec->mic_ch, 10);///调节录音mic的增益

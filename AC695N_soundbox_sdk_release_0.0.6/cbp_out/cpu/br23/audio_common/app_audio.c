...
static void audio_fade_timer(void *priv)
{
    ...
    u8 gain_rl = dac_hdl.vol_rl;
    u8 gain_rr = dac_hdl.vol_rr;
    ...
    if ((gain_l == __this->fade_gain_l) && (gain_r == __this->fade_gain_r)
        && (gain_rl == __this->fade_gain_rl) && (gain_rr == __this->fade_gain_rr)) {
        sys_hi_timer_del(__this->fade_timer);
        __this->fade_timer = 0;
        /*音量为0的时候mute住*/
        audio_dac_set_L_digital_vol(&dac_hdl, gain_l ? __this->digital_volume_fl : 0);
        audio_dac_set_R_digital_vol(&dac_hdl, gain_r ? __this->digital_volume_fr : 0);
        audio_dac_set_RL_digital_vol(&dac_hdl, gain_rl ? __this->digital_volume_rl : 0);
        audio_dac_set_RR_digital_vol(&dac_hdl, gain_rr ? __this->digital_volume_rr : 0);


        if ((gain_l == 0) && (gain_r == 0)  && (gain_rl == 0) && (gain_rr == 0)) {
    ...
    if (gain_rl > __this->fade_gain_rl) {
        gain_rl--;
    } else if (gain_rl < __this->fade_gain_rl) {
        gain_rl++;
    }

    if (gain_rr > __this->fade_gain_rr) {
        gain_rr--;
    } else if (gain_rr < __this->fade_gain_rr) {
        gain_rr++;
    }
...
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
    int err = audio_dac_set_analog_vol_support4way(&dac_hdl, gain_l, gain_r, gain_rl, gain_rr);
#else
...
static int audio_fade_timer_add(u8 gain_l, u8 gain_r, u8 gain_rl, u8 gain_rr)
{
...
static void audio_combined_fade_timer(void *priv)
{
    ...
    s16 dgain_rl = dac_hdl.d_volume[DA_REAR_LEFT];
    s16 dgain_rr = dac_hdl.d_volume[DA_REAR_RIGHT];
    ...
    __this->fade_dgain_step_rl = abs(dgain_rl - __this->fade_dgain_rl) / \
                                 (abs(gain_rl - __this->fade_gain_rl) + 1);
    if (__this->fade_dgain_step_rl > DGAIN_SET_MAX_STEP) {
        __this->fade_dgain_step_rl = DGAIN_SET_MAX_STEP;
    } else if (__this->fade_dgain_step_rl < DGAIN_SET_MIN_STEP) {
        __this->fade_dgain_step_rl = DGAIN_SET_MIN_STEP;
    }

    __this->fade_dgain_step_rr = abs(dgain_rr - __this->fade_dgain_rr) / \
                                 (abs(gain_rr - __this->fade_gain_rr) + 1);
    if (__this->fade_dgain_step_rr > DGAIN_SET_MAX_STEP) {
        __this->fade_dgain_step_rr = DGAIN_SET_MAX_STEP;
    } else if (__this->fade_dgain_step_rr < DGAIN_SET_MIN_STEP) {
        __this->fade_dgain_step_rr = DGAIN_SET_MIN_STEP;
    }

    /* log_info("<a:%d-%d-%d-%d-%d-%d-%d-%d d:%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d>\n", \ */
    /* gain_l, gain_r, __this->fade_gain_l, __this->fade_gain_r, \ */
    /* gain_rl, gain_rr, __this->fade_gain_rl, __this->fade_gain_rr, \ */
    /* dgain_l, dgain_r, __this->fade_dgain_l, __this->fade_dgain_r, \ */
    /* dgain_rl, dgain_rr, __this->fade_dgain_rl, __this->fade_dgain_rr, \ */
    /* __this->fade_dgain_step_l, __this->fade_dgain_step_r, \ */
    /* __this->fade_dgain_step_rl, __this->fade_dgain_step_rr); */

    local_irq_disable();

    if ((gain_l == __this->fade_gain_l) \
        && (gain_r == __this->fade_gain_r) \
        && (gain_r == __this->fade_gain_rl) \
        && (gain_r == __this->fade_gain_rr) \
        && (dgain_l == __this->fade_dgain_l)\
        && (dgain_l == __this->fade_dgain_r)\
        && (dgain_l == __this->fade_dgain_rl)\
        && (dgain_r == __this->fade_dgain_rr)) {
        sys_hi_timer_del(__this->fade_timer);
        __this->fade_timer = 0;
        /*音量为0的时候mute住*/
        if ((gain_l == 0) && (gain_r == 0) && (gain_rl == 0) && (gain_rr == 0)) {
        ...
        local_irq_enable();
        /* log_info("dac_fade_end,VOL:0x%x-0x%x-%d-%d-%d-%d-%d-%d-%d-%d\n", \ */
        /* JL_ANA->DAA_CON1, JL_AUDIO->DAC_VL0,  \ */
        /* __this->fade_gain_l, __this->fade_gain_r, \ */
        /* __this->fade_gain_rl, __this->fade_gain_rr, \ */
        /* __this->fade_dgain_l, __this->fade_dgain_r, \ */
        /* __this->fade_dgain_rl, __this->fade_dgain_rr); */
        return;
    }
    if ((gain_l != __this->fade_gain_l) \
        || (gain_r != __this->fade_gain_r)) {
...
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
        audio_dac_set_analog_vol_support4way(&dac_hdl, gain_l, gain_r, gain_rl, gain_rr);
#else
    ...
    if ((dgain_l != __this->fade_dgain_l) \
        ...
        if (gain_rl != __this->fade_gain_rl) {
            if (dgain_rl > __this->fade_dgain_rl) {
                if ((dgain_rl - __this->fade_dgain_rl) >= __this->fade_dgain_step_rl) {
                    dgain_rl -= __this->fade_dgain_step_rl;
                } else {
                    dgain_rl = __this->fade_dgain_rl;
                }
            } else if (dgain_rl < __this->fade_dgain_rl) {
                if ((__this->fade_dgain_rl - dgain_rl) >= __this->fade_dgain_step_rl) {
                    dgain_rl += __this->fade_dgain_step_rl;
                } else {
                    dgain_rl = __this->fade_dgain_rl;
                }
        ...
        if (gain_rr != __this->fade_gain_rr) {
            if (dgain_rr > __this->fade_dgain_rr) {
                if ((dgain_rr - __this->fade_dgain_rr) >= __this->fade_dgain_step_rr) {
                    dgain_rr -= __this->fade_dgain_step_rr;
                } else {
                    dgain_rr = __this->fade_dgain_rr;
                }
            } else if (dgain_rr < __this->fade_dgain_rr) {
                if ((__this->fade_dgain_rr - dgain_rr) >= __this->fade_dgain_step_rr) {
                    dgain_rr += __this->fade_dgain_step_rr;
                } else {
                    dgain_rr = __this->fade_dgain_rr;
                }
...
static int audio_combined_fade_timer_add(u8 gain_l, u8 gain_r, u8 gain_rl, u8 gain_rr)
{
...
void audio_fade_in_fade_out(u8 left_gain, u8 right_gain)
{
#if AUDIO_OUTPUT_INCLUDE_DAC
#if (SYS_VOL_TYPE == 0)
    __this->digital_volume_fl = left_gain * 16384 / get_max_sys_vol();
    ...
    //printf(">>>> audio_fade_in_fade_out %d %d %d\n", __this->digital_volume, left_gain, get_max_sys_vol());
    audio_dac_set_digital_vol(&dac_hdl, __this->digital_volume_fl);
    audio_fade_timer_add(MAX_ANA_VOL, MAX_ANA_VOL, MAX_ANA_VOL, MAX_ANA_VOL);
#elif (SYS_VOL_TYPE == 1)
    audio_fade_timer_add(left_gain, right_gain, left_gain, right_gain);
#elif (SYS_VOL_TYPE == 2)
    audio_combined_fade_timer_add(left_gain, right_gain, left_gain, right_gain);
#elif (SYS_VOL_TYPE == 3)
    audio_fade_timer_add(MAX_ANA_VOL, MAX_ANA_VOL, MAX_ANA_VOL, MAX_ANA_VOL);
#endif
#endif
...
#if (defined(TONE_MODE_DEFAULE_VOLUME)  && (TONE_MODE_DEFAULE_VOLUME != 0))
        app_var.wtone_volume = TONE_MODE_DEFAULE_VOLUME;
        volume = app_var.wtone_volume;
        break;
#endif
#if (defined(APP_AUDIO_STATE_WTONE_BY_MUSIC) && (APP_AUDIO_STATE_WTONE_BY_MUSIC == 1))
...
#if (AUDIO_OUTPUT_WAY == AUDIO_OUTPUT_WAY_BT)
        bt_emitter_set_vol(0);
#endif
#else
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
        audio_dac_set_analog_vol_support4way(&dac_hdl, 0, 0, 0, 0);
#else
...
void app_audio_state_switch(u8 state, s16 max_volume)
{
    ...
    if (__this->state == APP_AUDIO_STATE_CALL) {
        ...
        /*调数字音量的时候，模拟音量定最大*/
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR)
        audio_dac_set_analog_vol_support4way(&dac_hdl, max_volume, max_volume, max_volume, max_volume);
#else

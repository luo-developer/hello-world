...
static const u8 alink0_IOS_portB[] = {
    IO_PORTB_15, IO_PORTB_09, IO_PORTB_10, IO_PORTB_11, IO_PORTB_12, IO_PORTB_13, IO_PORTB_14,
};
...
int iis_open(iis_param *param)
{
    ...
    case ALINK1_PORTA: //MCLK:PB0  SCLK:PC0  LRCK:PC1  CH0:PC2  CH1:PC3  CH2:PC4  CH3:PC5
        if (param->soe) {
            gpio_set_direction(alink1_IOS_portA[IIS_IO_SCLK], 0);
            gpio_set_direction(alink1_IOS_portA[IIS_IO_LRCK], 0);
            gpio_set_die(alink1_IOS_portA[IIS_IO_SCLK], 1);
            gpio_set_die(alink1_IOS_portA[IIS_IO_LRCK], 1);
        }
        if (param->moe) {
            gpio_set_direction(alink1_IOS_portA[IIS_IO_MCLK], 0);
            gpio_set_die(alink1_IOS_portA[IIS_IO_MCLK], 1);
        }
        for (i = 0; i < ALINK_CH_MAX; i++) {
            if (param->ch[i].enable) {
                gpio_set_direction(alink1_IOS_portA[IIS_IO_CH0 + i], param->ch[i].dir);
                gpio_set_die(alink1_IOS_portA[IIS_IO_CH0 + i], 1);
                ALINK_SET_ADR(module, i, (volatile unsigned int)param->ch[i].dma_adr);
            }
        }
    ...
    audio_link_switch_sr(param->port, param->rate);
    iis_ch_en(param);

    ALINK_EN(module, 1);
#if 0
    if (!module) {
        alink_printf("ALINK0_CON0:0x%x", JL_ALNK0->CON0);
        alink_printf("ALINK0_CON1:0x%x", JL_ALNK0->CON1);
        alink_printf("ALINK0_CON2:0x%x", JL_ALNK0->CON2);
        alink_printf("ALINK0_CON3:0x%x", JL_ALNK0->CON3);
    } else {
...
void fade_buf(s16 *data, u32 len, u8 inout, u8 ch_num)
{
    s32 i;
    s16 last_data[2];
    s32 t_data = 0;
    s32 points = len >> 1;
    if (inout) {
        memset(data, 0x00, points);
        for (i = points / 2; i < points; i += ch_num) {
            t_data = (s32)data[i];
            t_data = (s32)(t_data * (i - points / 2) * (i - points / 2) / points / points);
            data[i] = (s16)t_data;
            if (ch_num > 1) {
                t_data = (s32)data[i + 1];
                t_data = (s32)(t_data * (i - points / 2) * (i - points / 2) / points / points);
        ...
        last_data[0] = data[0];
        last_data[1] = data[1];
        for (i = 0; i < points; i += ch_num) {
            t_data = (s32)data[0];
            t_data = (s32)(t_data - (t_data * i / points));
            data[i] = (s16)t_data;
            if (ch_num > 1) {
                t_data = (s32)data[1];
                t_data = (s32)(t_data - (t_data * i / points));
...
void set_iis_buf(struct s_alink_hdl *hdl, cbuffer_t *pcbuf, u8 *buf, u32 len)
{
...
#if PCM_ENC_ENABLE
    struct s_pcm_enc *enc = hdl->enc;
    if (enc != NULL) {
        int rlen = cbuf_get_data_len(pcbuf);
        /* s16 *data = buf; */
        /* printf("==C%d %d %d %d %d\n", \ */
        /* rlen, \ */
        /* data[0], \ */
        ...
        if (enc->rdoor && (rlen < enc->begin_size)) {
            memset(buf, 0x00, len);
            return;
        }

        if (rlen < enc->bottom_size) {
            enc->audio_new_rate += PCM_RATE_INC_STEP;
        }

        if (rlen > enc->top_size) {
            enc->audio_new_rate -= PCM_RATE_DEC_STEP;
        }

        if (rlen > len) {
            rlen = cbuf_read(pcbuf, buf, len);
            if (enc->rdoor) {
                fade_buf((s16 *)buf, len, 1, 2);
            }
            enc->rdoor = 0;
        } else {
            printf("iis end %d %d\n", len, rlen);
            rlen = cbuf_read(pcbuf, buf, rlen);
            fade_buf((s16 *)buf, 64, 0, 2);
            memset(buf + 64, 0x00, len - 64);
            enc->rdoor = 1;
        }
        return;
    }
#endif
    int rlen = 0;
    if (hdl->sync_start && !audio_iis_sync_align_start(&iis_hdl)) {
        rlen = cbuf_read(pcbuf, buf, len > hdl->before_sync_len ? hdl->before_sync_len : len);
        if (rlen < len) {
            memset((u8 *)buf + rlen, 0x0, len - rlen);
        }
        hdl->before_sync_len -= rlen;
        return;
    }

    int data_len = cbuf_get_data_len(pcbuf);
    if (data_len > len) {
        data_len = len;
    }
    rlen = cbuf_read(pcbuf, buf, data_len);
    if (rlen == len) {
        /* putbyte('o'); */
    } else {
        memset((u8 *)buf + rlen, 0x0, len - rlen);
...
void iis0_cb(u8 ch_idx, void *buf, u32 len)
{
    ...
    if (iis_module_data[0].user_data.ch[ch_idx].dir == ALINK_DIR_TX) {
        if (p_alink_hdl[0]->param.dsp == 0) { //只支持立体声
            set_iis_buf(p_alink_hdl[0], &p_alink_hdl[0]->pcm_cbuf[ch_idx], buf, len * 2 * 2);
            /* putbyte('0'); */
        } else { //单声道
        ...
        audio_iis_sync_len_update(&iis_hdl, len);
    } else {
...
void iis1_cb(u8 ch_idx, void *buf, u32 len)
{
    ...
    if (iis_module_data[1].user_data.ch[ch_idx].dir == ALINK_DIR_TX) {
        if (p_alink_hdl[1]->param.dsp == 0) { //只支持立体声
            set_iis_buf(p_alink_hdl[1], &p_alink_hdl[1]->pcm_cbuf[ch_idx], buf, len * 2 * 2);
            /* putbyte('1'); */
...
#define CH_CBUF_DATA_LEN   (1024 * 8)
#define IIS_POINT_NUM      (256)
#if 0
...
int audio_link_write_stereodata_end(s16 *buf, int len, u8 alink_port)
{
    ...
    if (wlen != len) {
        /* log_e("iis len %d , wlen %d\n", len, wlen); */
    }
    return wlen;
}

static int audio_link_output_buffer_handler(void *priv, u8 state, struct audio_iis_buffer *buf)
{
    struct s_alink_hdl *alink_hdl = (struct s_alink_hdl *)priv;
    cbuffer_t *cbuf = NULL;
    int i = 0;
    for (i = 0; i < ALINK_CH_MAX; i++) {
        cbuf = &alink_hdl->pcm_cbuf[i];
        if (cbuf) {
            break;
        }
    }
    ...
    if (state == AUDIO_IIS_GET_OUTPUT_BUFFER) {
        int irq_space = (alink_hdl->param.dsp ? 1 : 2) * alink_hdl->param.frame_len;
        if (!cbuf_is_write_able(cbuf, irq_space)) {//少于一次irq数据量不写入，保留空间
            buf->addr = NULL;
            buf->len = 0;
            return -ENOMEM;
        }
        buf->addr = cbuf_write_alloc(cbuf, &buf->len);
        if ((u32)buf->addr % 4) {
            log_e("cbuf wr ptr error : 0x%x\n", buf->addr);
            cbuf_write_updata(cbuf, 4 - ((u32)buf->addr % 4));
            cbuf_read_updata(cbuf, 4 - ((u32)buf->addr % 4));
            buf->addr = cbuf_write_alloc(cbuf, &buf->len);
        }
        if (buf->len > irq_space) {
            buf->len -= irq_space;
        }
    } else if (state == AUDIO_IIS_PUT_OUTPUT_BUFFER) {
        cbuf_write_updata(cbuf, buf->len);
    }
...
void audio_link_sync_start(u8 alink_port)
{
    ...
    if (p_alink_hdl[module]) {
        u8 channel = p_alink_hdl[module]->param.dsp ? 1 : 2;
        audio_iis_sync_connect_open(&iis_hdl, p_alink_hdl[module], audio_link_output_buffer_handler);
        audio_iis_sync_set_channel(&iis_hdl, channel);
        audio_iis_sync_start(&iis_hdl);
        int max_points = CH_CBUF_DATA_LEN / channel / 2;
        int start_points = ((TCFG_IIS_OUTPUT_SR / (1000 / 30)) >> 2) << 2;
        if (start_points > max_points - IIS_POINT_NUM) {
            start_points = max_points - IIS_POINT_NUM;
        }
        audio_iis_sync_set_start_points(&iis_hdl, start_points);
        local_irq_disable();
        cbuffer_t *cbuf = NULL;
        int i = 0;
        p_alink_hdl[module]->before_sync_len = 0;
        p_alink_hdl[module]->sync_start = 1;
        for (i = 0; i < ALINK_CH_MAX; i++) {
            cbuf = &p_alink_hdl[module]->pcm_cbuf[i];
            if (cbuf) {
                break;
            }
        }
        if (cbuf) {
            p_alink_hdl[module]->before_sync_len = cbuf_get_data_len(cbuf);
        }
        local_irq_enable();
    }
}
...
int audio_link_sync_stop(u8 alink_port)
{
    ...
    if (p_alink_hdl[module]) {
        audio_iis_sync_stop(&iis_hdl);
        p_alink_hdl[module]->sync_start = 0;
    }

    return 0;
}

int audio_link_sync_reset(u8 alink_port)
{
    u8 module = ALINK_PORT_TO_MODULE(alink_port);
...
int audio_link_write_stereodata(s16 *buf, int len, u8 alink_port)
{
    ...
    if (p_alink_hdl[module]->sync_start) {
        return audio_iis_sync_write(&iis_hdl, buf, len);
    }

    return audio_link_write_stereodata_end(buf, len, alink_port);
}

int audio_link_clear_stereodata(u8 alink_port)
{
    u8 module = ALINK_PORT_TO_MODULE(alink_port);
    int wlen = 0;
    if (!p_alink_hdl[module]) {
    ...
    if (ch_en_bitmap[module]&BIT(0)) {
        cbuf_clear(&p_alink_hdl[module]->pcm_cbuf[0]);
    }
    if (ch_en_bitmap[module]&BIT(1)) {
        cbuf_clear(&p_alink_hdl[module]->pcm_cbuf[1]);
    }
    if (ch_en_bitmap[module]&BIT(2)) {
        cbuf_clear(&p_alink_hdl[module]->pcm_cbuf[2]);
    }
    if (ch_en_bitmap[module]&BIT(3)) {
        cbuf_clear(&p_alink_hdl[module]->pcm_cbuf[3]);
    }
...
int audio_link_open(u8 alink_port, u8 dir)
{
...
#if PCM_DEC_ENABLE
        //open pcm dec
        dec = malloc(sizeof(struct s_pcm_dec));
        if (!dec) {
    ...
    for (i = 0; i < ALINK_CH_MAX; i++) {
        if (BIT(i)&ch_en_bitmap[module]) {
            ...
            printf("audio link bufffer init : 0x%x, 0x%x\n", alink_hdl->p_data_buf[i], alink_hdl->p16_dma_buf[i]);
            ...
            if (alink_hdl->param.dsp) {
                ...
                audio_iis_set_channel(&iis_hdl, 1);
            } else {
                alink_hdl->param.ch[i].ch_md = ALINK_MD_IIS;
                audio_iis_set_channel(&iis_hdl, 2);
            }

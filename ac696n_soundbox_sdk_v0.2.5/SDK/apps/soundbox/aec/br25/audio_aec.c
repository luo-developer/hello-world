 ...
 *0:Normal malloc
 *1:LMP rx_buf malloc
 *2:aec_overlay malloc
 */
#define MALLOC_MULTIPLEX_EN	    2
#if (MALLOC_MULTIPLEX_EN == 2)
u8 aec_mux_mem[7196] sec(.aec_mem);
#endif
extern void *lmp_malloc(int);
extern void lmp_free(void *);
void *zalloc_mux(int size)
{
#if (MALLOC_MULTIPLEX_EN == 1)
    void *p = NULL;
    ...
    return p;
#elif (MALLOC_MULTIPLEX_EN == 2)
    //printf("[malloc_overlay]p = 0x%x,aec_mux_mem size = %d,malloc_size:%d\n",aec_mux_mem,sizeof(aec_mux_mem),size);
    ASSERT(size <= sizeof(aec_mux_mem));
    memset(aec_mux_mem, 0, sizeof(aec_mux_mem));
    return aec_mux_mem;
#else
    return zalloc(size);
#endif
...
extern const int aec_coeff_8k_highpass_ul[];
/*upstream high-pass(200) for 16k*/
extern const int aec_coeff_16k_highpass_ul[];
/*upstream high-pass(200) and high-shelf(4000,-8db) for 16k*/
extern const int aec_coeff_16k_hp_and_hs_ul[];
int aec_ul_eq_filter(int sr, struct audio_eq_filter_info *info)
{
    r_printf("ul_eq sr:%d", sr);
    if (sr == 16000) {
        info->L_coeff = info->R_coeff = (void *)aec_coeff_16k_highpass_ul;
    } else {
        info->L_coeff = info->R_coeff = (void *)aec_coeff_8k_highpass_ul;
    }
...
static void audio_aec_param_read_config(struct aec_s_attr *p)
{
    ...
    if (ret == sizeof(AEC_CONFIG)) {
        ...
        p->AGC_NDT_fade_in_step = cfg.fade_gain;
        p->AGC_NDT_fade_out_step = cfg.fade_gain;
        p->AGC_DT_fade_in_step = cfg.fade_gain;
        p->AGC_DT_fade_out_step = cfg.fade_gain;
...
int audio_aec_init(u16 sample_rate)
{
    ...
    if (aec_param->ul_eq_en) {
        ...
        ul_eq_param.online_en = 0;
        ul_eq_param.mode_en = 0;
        ul_eq_param.remain_en = 0;
        ul_eq_param.max_nsection = 3;

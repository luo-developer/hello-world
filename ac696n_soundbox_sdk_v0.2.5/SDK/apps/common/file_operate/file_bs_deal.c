...
u32 file_bs_open_handle(FILE_BS_DEAL *fil_bs, u8 *ext_name)
{
    ...
    fset_ext_type(fil_bs->dev->root_path, ext_name);

    total_dir = fopen_dir_info(fil_bs->dev->root_path, &fil_bs->file, 0);
    if (fil_bs->file == 0) {
...
u32 file_bs_entern_dir(FILE_BS_DEAL *fil_bs, FS_DIR_INFO *dir_info)
{
    ...
    total_dir = fopen_dir_info(fil_bs->dev->root_path, &fil_bs->file, dir_info); //
...
void file_bs_test(void)
{
    ...
    int i, j, cnt, total;
    total = file_bs_open_handle(fil_bs, "MP3");

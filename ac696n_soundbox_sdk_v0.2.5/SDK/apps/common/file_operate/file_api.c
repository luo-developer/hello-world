...
FILE *file_api_auto_create_file(const char *path, const char *folder, const char *filename, u32 *file_index)
{
    ...
    extern int sdfile_rec_scan_ex();
    /* extern void rec_clear_norfs_fileindex(void); //内置flash录音文件头全擦 */
    /* rec_clear_norfs_fileindex(); */
    sdfile_rec_index = sdfile_rec_scan_ex();
    last_num = sdfile_rec_index + 1;
#else

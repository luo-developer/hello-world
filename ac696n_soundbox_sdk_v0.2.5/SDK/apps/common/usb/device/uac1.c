...
static const u8 uac_spk_ac_standard_interface_desc[] = {
...
///Type 1 Format type descriptor
    ...
    (((u16)SPK_AUDIO_RATE * 100) & 0xFF), (((u16)SPK_AUDIO_RATE * 100) >> 8), 0x00, //48K(0x00BB80)
...
///standard interface AC descriptor(Interface 0, Alternate Setting 0)
static const u8 uac_mic_ac_interface_desc[] = {
    ...
    0x00,       //AlternateSetting
    0x00,       //NumEndpoint
    USB_CLASS_AUDIO,       //InterfaceClass
    USB_SUBCLASS_AUDIOCONTROL,       //InterfaceSubClass
    0x00,       //InterfaceProtocol
    MIC_STR_INDEX,      //Interface String
...
#define MIC_CONTROLS_SIZE	1
    7 + (1 + 1) *MIC_CONTROLS_SIZE,    	//Length
    USB_DT_CS_INTERFACE,      //DescriptorType:audio interface descriptor
    UAC_FEATURE_UNIT,       //DescriptorSubType:Audio Feature Unit
    MIC_FEATURE_UNIT_ID,       //UnitID
    MIC_INPUT_TERMINAL_ID,      //SourceID: #Microphone
    MIC_CONTROLS_SIZE,      //ControlSize:1 byte
    0x03,      //bmaControls:Mute,volume
    0x00,       //Controls(0):Left Volume
    0x00,       //Feature String
};
///USB Microphone OT
...
///Type 1 Format type descriptor
    ...
    0x01,      //SampleFreqType:One sampling frequency.
    (((u16)MIC_AUDIO_RATE * 100) & 0xFF), (((u16)MIC_AUDIO_RATE * 100) >> 8), 0x00, //48K(0x00BB80)
...
static u32 uac_vol_handler(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case UAC_FU_MUTE:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            ...
            if (mute) {
                uac_mute_volume(SPK_FEATURE_UNIT_ID, 0);
                uac_info->spk_mute = 1;
            } else {
                uac_mute_volume(SPK_FEATURE_UNIT_ID, uac_info->spk_vol);
                uac_info->spk_mute = 0;
            }
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            memcpy((u8 *)&mute, read_ep, 1);
            if (mute) {
                uac_mute_volume(MIC_FEATURE_UNIT_ID, 0);
                uac_info->mic_mute = 1;
            } else {
                uac_mute_volume(MIC_FEATURE_UNIT_ID, uac_info->spk_vol);
                uac_info->mic_mute = 0;
            }
        }
        break;
    case UAC_FU_VOLUME:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            memcpy((u8 *)&uac_info->spk_vol, read_ep, sizeof(uac_info->spk_vol));
            uac_mute_volume(SPK_FEATURE_UNIT_ID, uac_info->spk_vol);
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            memcpy((u8 *)&uac_info->mic_vol, read_ep, sizeof(uac_info->spk_vol));
            uac_mute_volume(MIC_FEATURE_UNIT_ID, uac_info->mic_vol);
        }
        break;
    default :
...
#endif
static u32 speaker_ac_itf_handler(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    u8 mute;
    ...
    case UAC_GET_CUR:
        ...
        case UAC_FU_VOLUME:
            if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
                tx_payload = (u8 *)&uac_info->spk_vol;
            } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    u8 *ep_buffer = usb_get_ep_buffer(usb_id, SPK_ISO_EP_OUT);
    uac_speaker_stream_open(SPK_AUDIO_RATE * 100, SPK_CHANNEL);
...
static u32 spk_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u32 tx_len;
    u8 *tx_payload = usb_get_setup_buffer(usb_device);
...
static void mic_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    usb_g_iso_write(usb_id, MIC_ISO_EP_IN, NULL, MIC_FRAME_LEN);
}
static void open_mic(struct usb_device_t *usb_device)
{
    ...
    uac_mic_stream_open(MIC_AUDIO_RATE * 100, MIC_FRAME_LEN, MIC_CHANNEL);
...
static u32 mic_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u8 *tx_payload = usb_get_setup_buffer(usb_device);
    u32 tx_len;
...
u32 uac_spk_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    log_info("spkdc:%d\n", *cur_itf_num);
    memcpy(tptr, (u8 *)uac_spk_ac_standard_interface_desc, sizeof(uac_spk_ac_standard_interface_desc));
    tptr[2] = *cur_itf_num;
    tptr += sizeof(uac_spk_ac_standard_interface_desc);//0x09
    ...
    if (usb_set_interface_hander(usb_id, *cur_itf_num, speaker_ac_itf_handler) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }

    (*cur_itf_num) ++;
    if (usb_set_interface_hander(usb_id, *cur_itf_num, spk_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }
...
u32 uac_mic_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    memcpy(tptr, (u8 *)uac_mic_ac_interface_desc, sizeof(uac_mic_ac_interface_desc));
    tptr[2] = *cur_itf_num;
    tptr += sizeof(uac_mic_ac_interface_desc);//0x09
    ...
    (*cur_itf_num) ++;
    if (usb_set_interface_hander(usb_id, *cur_itf_num, mic_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac mic set interface_hander fail");
    }
...
#else
    u32 reg;
    reg = usb_read_txcsr(usb_id, MIC_ISO_EP_IN);
    reg |= TXCSRP_ClrDataTog | TXCSRP_FlushFIFO;
    usb_write_txcsr(usb_id, MIC_ISO_EP_IN, reg);

    reg = usb_read_rxcsr(usb_id, SPK_ISO_EP_OUT);
    reg |= RXCSRP_ClrDataTog | RXCSRP_FlushFIFO;
    usb_write_rxcsr(usb_id, SPK_ISO_EP_OUT, reg);
#endif
}
u32 uac_audio_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    log_info("audio control interface num:%d\n", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, speaker_ac_itf_handler) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }
    ...
    (*cur_itf_num) ++;
    log_info("speaker stream interface num:%d\n", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, spk_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }

    (*cur_itf_num) ++;
    log_info("mic stream interface num:%d\n", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, mic_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac mic set interface_hander fail");
    }
...
u32 uac_register()
{
    ...
    uac_info = &_uac_info;
#endif
    uac_info->spk_vol = uac_get_spk_vol();
    uac_info->max_vol = 99;
    uac_info->min_vol = 0;
    uac_info->def_vol = 49;
    uac_info->vol_res = 1;

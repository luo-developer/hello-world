...
#if (defined MUTIl_CHARGING_BOX_EN) && (MUTIl_CHARGING_BOX_EN)
    buf[8] = 0x21;	//   2:693x   |  protocol verson
#else
    buf[8] = 0x20;	//   2:693x   |  protocol verson
#endif
...
static int update_adv_data(u8 *buf)
{
    ...
    buf[6] = 0x20;	//   2:693x   |  protocol verson
...
#if (defined MUTIl_CHARGING_BOX_EN) && (!MUTIl_CHARGING_BOX_EN) && (RCSP_ADV_EN) && (TCFG_USER_TWS_ENABLE)
    if (con_handle && JL_rcsp_get_auth_flag()) {
        extern void deal_adv_setting_gain_time_stamp(void);
        deal_adv_setting_gain_time_stamp();
    }
#endif

    if (con_handle && __this->ble_adv_notify &&  JL_rcsp_get_auth_flag()) {
...
{
    if (get_ble_work_state() == BLE_ST_CONNECT) {
        ble_disconnect(NULL);
    }
}

...
int last_enc_file_path_get(char path[64])
{
    ...
    extern int sdfile_rec_index;
    nor_fs_index = sdfile_rec_index;
#endif
...
#if FLASH_INSIDE_REC_ENABLE
    sprintf(path, "%s%s%s%s", MOUNT_SDFILE_REC_PATH"/C/", "JL_REC/AC69", index_str, ".WAV");
#else
    ...
    char index_str[5] = {0};
    struct storage_dev *last_dev = storage_dev_last();
    printf("enc file idex = %d, dev_path = %s\n", enc_file_index, last_dev->root_path);
    if (enc_file_index != (u32) - 1) {
        ...
        if (enc_file_coding_type == AUDIO_CODING_WAV) {
            sprintf(path, "%s%s%s%s", last_dev->root_path, "JL_REC/AC69", index_str, ".WAV");
        } else if (enc_file_coding_type == AUDIO_CODING_G726) {
            sprintf(path, "%s%s%s%s", last_dev->root_path, "JL_REC/AC69", index_str, ".wav");
        } else if (enc_file_coding_type == AUDIO_CODING_MP3) {
            sprintf(path, "%s%s%s%s", last_dev->root_path, "JL_REC/AC69", index_str, ".MP3");
        } else {
            return -1;
        }
        printf("rec file =%s\n", path);
...
#if TCFG_NOR_FS_ENABLE || FLASH_INSIDE_REC_ENABLE
                extern int max_rec_capacity;
                extern int phone_max_rec_capacity;
#if FLASH_INSIDE_REC_ENABLE
                max_rec_capacity = phone_max_rec_capacity;
#endif
                if (wfil->file_size + len > (max_rec_capacity - 4096)){
...
void enc_write_file_close(void *hdl)
{
    ...
    if (wfil->file) {
        ...
        do {
            ...
            fclose(wfil->file);
#if (FLASH_INSIDE_REC_ENABLE == 0)
            file_opr_available_dev_online(wfil->dev);
#endif
        } while (0);
        if (wfil->evt_cb) {
    ...
    /* if (wfil->fmnt) { */
    /* unmount(wfil->dev->storage_path); */
    /* wfil->fmnt = NULL; */
    /* } */
    free(wfil);
}

void *enc_write_file_open(struct storage_dev *dev, const char *folder, const char *filename)
{
    if (!dev || !folder || !filename) {
        return NULL;
    }
    int err;
    ...
    /* wfil->fmnt = mount(dev->dev_name, dev->storage_path, dev->fs_type, 3, NULL); */
    /* if (!wfil->fmnt) { */
    /* log_e("dev mount err "); */
    /* goto _exit; */
    /* } */
    wfil->file = file_api_auto_create_file(dev->root_path, folder, filename, &enc_file_index);
    if (!wfil->file) {
...
void *get_wfil_hdl(void *enc_whdl)
{
...
void enc_w_test(void)
{
    ...
    printf("%s,%d ", __func__, __LINE__);
    tst->hdl = enc_write_file_open(storage_dev_last(), "/JL_REC", "AC690000.MP3");
    printf("%s,%d ", __func__, __LINE__);
    if (!tst->hdl) {
        goto __exit;
    }
